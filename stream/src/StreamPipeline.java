...
// Pipeline operations not chained
        ...
        personStream = personStream.limit(5);
        personStream = personStream.filter(
                (s) -> s.getName().equals("Mary"));
        long maryCount = personStream.count();
...
// Declaring Stream using generate with Supplier lambda expression
        //this stream doesn't end. anyMatch is short-Circuiting, but it is not enough to terminate the stream.
...
//                        (s)-> s.getName().equals("Aaron123")));
    }
}
/**
 ---- Process pipeline for First, Second ----
 First
 Second
 ---- Created a personStream variable ----
 Just looping for the heck of it
 Just looping for the heck of it
 Just looping for the heck of it
 ------ Adding intermediate operation to pipeline -----
 Again looping for the heck of it
 Again looping for the heck of it
 Again looping for the heck of it
 ---- Process pipeline for personStream ----
 Instantiating Mary
 [Mary]
 Instantiating Joe
 [Joe]
